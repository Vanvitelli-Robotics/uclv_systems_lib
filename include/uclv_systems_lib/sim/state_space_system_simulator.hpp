...
class StateSpaceSystemSimulator : public SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>
{
public:
  ...
  typedef ::uclv::systems::StateSpaceInterface<dim1_state, dim1_input, dim1_output, dim2_state, dim2_input, dim2_output>
      StateSpaceInterface;
...
public:
  ...
  StateSpaceSystemSimulator(typename StateSpaceInterface::SharedPtr state_space_system_ptr)
    : SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>(state_space_system_ptr)
  ...
  {
    SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>::reset_history();
    state_history_.clear();
  }

  virtual inline void reserve_history(std::size_t size) override
  {
    SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>::reserve_history(size);
    state_history_.reserve(size);
  }

  const std::vector<Eigen::Matrix<double, dim1_state, dim2_state>>& get_state_history() const
  {
    return state_history_;
  }

  void reset(const Eigen::Ref<const Eigen::Matrix<double, dim1_state, dim2_state>>& x0)
  {
    reset_history();
    state_space_system->set_state(x0);
  }

  const Eigen::Matrix<double, dim1_state, dim2_state>& get_initial_state() const
  {
  ...
  const Eigen::Matrix<double, dim1_state, dim2_state>& get_final_state() const
  {
    if (state_history_.empty())
    {
      return state_space_system->get_state();
  ...
  virtual inline void simulate(const Eigen::Ref<const Eigen::Matrix<double, dim1_input, dim2_input>>& u_k) override
  {
    ...
    SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>::simulate(u_k);
    state_history_.push_back(state_space_system->get_state());
  }

  virtual void disp_sim() const override
  {
    SystemSimulator<dim1_input, dim1_output, dim2_input, dim2_output>::disp_sim();
...
protected:
  std::vector<Eigen::Matrix<double, dim1_state, dim2_state>> state_history_;
  Eigen::Matrix<double, dim1_state, dim2_state> x0_;
};
}  // namespace uclv::systems
